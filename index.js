...
app.post('/merge', upload.array('files', 100), (req, res) => {
  ...
  if (req.files) {
    ...
    exec(`magick convert ${list} -resize 500x600 \ -gravity center \ -extent 1240x1753 -units PixelsPerInch -density 150x150 -quality ${range_value} -page ${page_size}+0+0 ${outputFilePath}`, (err, stdout, stderr) => {
      if (err) throw err
