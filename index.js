...
app.post('/merge', upload.array('files', 100), (req, res) => {
  ...
  if (req.files) {
    ...
    exec(`magick convert ${list} -background white -gravity Center -border 30 -bordercolor white -quality ${range_value} -page ${page_size} ${outputFilePath}`, (err, stdout, stderr) => {
      if (err) throw err
